...
module I18n
  ...
  ERRORS = {
    "zh-TW" => {
      ...
      "disposable_email"    => "拋棄式（暫時）信箱不予受理，請改用常用信箱",
      "verification_invalid" => "驗證連結無效或已使用過",
      "verification_expired" => "驗證連結已過期，請重新申請",
    ...
    "en" => {
      ...
      "disposable_email"    => "Disposable/temporary email addresses are not accepted; please use a regular inbox",
      "verification_invalid" => "Verification link is invalid or already used",
      "verification_expired" => "Verification link has expired; please sign up again",
    ...
    "ja" => {
      ...
      "disposable_email"    => "使い捨て（一時）メールアドレスは利用できません。通常のメールをご利用ください",
      "verification_invalid" => "確認リンクが無効か、すでに使用されています",
      "verification_expired" => "確認リンクの有効期限が切れています。もう一度申請してください",
    ...
    "ko" => {
      ...
      "disposable_email"    => "일회용(임시) 이메일은 사용할 수 없습니다. 일반 이메일을 사용해 주세요",
      "verification_invalid" => "인증 링크가 유효하지 않거나 이미 사용되었습니다",
      "verification_expired" => "인증 링크가 만료되었습니다. 다시 신청해 주세요",
